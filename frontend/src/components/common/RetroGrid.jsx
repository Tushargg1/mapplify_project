...
export function RetroGrid({
  ...
  useEffect(() => {
    ...
    if (syncSceneRef.current) syncSceneRef.current();
  ...
  return (
    <div
      ref={containerRef}
      className={cls('pointer-events-none absolute size-full overflow-hidden', className)}
      style={gridStyles}
      {...props}
    >
      ...
      <canvas
        ref={canvasRef}
        className={cls('absolute inset-0 size-full', isWebGlReady ? 'opacity-100' : 'opacity-0')}
      />
